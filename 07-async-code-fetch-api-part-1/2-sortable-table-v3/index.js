...
export default class SortableTable {
  ...
  createEndpoint(id, order, start, end) {
    return `${BACKEND_URL}/${this.url}?_embed=subcategory.category&_sort=${id}&_order=${order}&_start=${start}&_end=${end}`
  }
  async loadData(id, order) {
    ...
    const data = await fetchJson(this.createEndpoint(id, order, start, end));
    return data;
  }
